...
class DrivableMap {
     ...
     */
     ...
     public boolean addDrivable(String id, Drivable ele) {
        if (!this.drivable_map.containsKey(id)) {
            this.drivable_map.put(id, ele);
            return true;
        } else {
            return false;
        }
     ...
     */
     ...
     public List<Tradable> getTradable() {
        List<Tradable> Permitted = new ArrayList<Tradable>();
        for (Drivable item: this.drivable_map.values()) {
            if (item instanceof Tradable) {
                Permitted.add((Tradable) item);
            }
        }
        return Permitted;
     }
